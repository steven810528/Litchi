...
public class SetAdvancedSharecodeTask extends
		AsyncTask<FsInfo, Integer, Integer> {
	private static final String TAG = "SetAdvancedSharecodeTask";
	...
	public SetAdvancedSharecodeTask(Context ctx, ApiConfig apiCfg) {
		this.ctx = ctx;
		this.apiCfg = apiCfg;
	}
	...
	protected Integer doInBackground(FsInfo... fInfo) {
		...
		SetAdvancedSharecodeHelper helper = new SetAdvancedSharecodeHelper(
				"1".equals(fInfo[0].entryId) ? "0" : "1", fInfo[0].entryId,
				null, "1");
		
		try {
			SetAdvancedSharecodeResponse response=(SetAdvancedSharecodeResponse)helper.process(apiCfg);
			if ( response == null )
			{
