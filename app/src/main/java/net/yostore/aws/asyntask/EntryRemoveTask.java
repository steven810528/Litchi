...
public class EntryRemoveTask extends AsyncTask<FsInfo, Integer, Integer>
{
	private static final String TAG = "EntryRemoveTask";
	...
	public EntryRemoveTask(Context ctx, ApiConfig apiCfg)
	{
		this.ctx 	= ctx;
		this.apiCfg = apiCfg;		
	}
	...
	protected Integer doInBackground(FsInfo... fInfo)
	{
		...
		{
			typeName = "File";
			helper	 = new FileRemoveHelper(fInfo[0].entryId);
		}
		else if ( fInfo[0].entryType == EntryType.Folder )
		{
			typeName = "Folder";
			helper   = new FolderRemoveHelper(fInfo[0].entryId);
		}
		else
		{
		...
		msg.append("Removing ").append(typeName).append("...");
	...
	protected void onProgressUpdate(Integer... values)
	{
		...
		if ( values[0] == 0 )
		{
			try
			{
				_mdialog = ProgressDialog.show(ctx, ctx.getString(R.string.app_name), "Removing " + typeName + "...", true, true, new OnCancelListener() {
					@Override
	...
	{
		String msg = typeName + " has been removed!";
	...
	{		
		...
		MessageDialog.show(ctx, "Removing " + typeName, msg.toString());
	}
}
